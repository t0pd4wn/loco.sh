...
set_ubuntu_style(){
  ...
  if [[ "${SHORT_OS_VERSION}" == "21" ]] || [[ "${SHORT_OS_VERSION}" == "22" ]]; then
    ...
    cmd::record "gsettings set org.gnome.shell.extensions.dash-to-dock dock-position TOP"
    cmd::record "gsettings set org.gnome.shell.extensions.dash-to-dock dash-max-icon-size 40"
